...
/**
 * One Запис open at a time on a Записи tab, and every answer about a Запис landing where it
 * belongs while the tab moves on under it: another Запис opened, the open one closed, the filters
 * changed.
 *
 ...
 * another Запис's details, never reopens a closed one and never reaches a list read since.
 *
 * The Салон's tab carries the whole matrix. The Незалежний майстер's and the Майстер салону's each
 * prove their own wiring with one case.
 *
 * Every late answer is held by the mock and let go at a chosen moment. Where a spec has to show
 * that a late answer changed nothing, the card makes a request of its own after it — the
 * Блокування dialog states the upcoming count — and waits for that one to be drawn: by then the
 * late answer has had its chance.
 */
