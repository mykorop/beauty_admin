...
@Component({
  ...
  template: `
    @if (rows(); as rows) {
      <table class="w-full max-w-5xl rounded-lg border border-slate-200 bg-white text-left text-sm">
        ...
        <tbody>
          @for (row of rows; track row.invite.inviteId) {
            <tr class="border-b border-slate-100 align-top last:border-0" data-testid="invite-row">
              ...
              <td class="px-4 py-3">
                <p-tag data-testid="invite-status" [severity]="row.statusSeverity" [value]="row.statusKey | t" />
                @if (row.invite.deliveryFailed) {
                  <div class="mt-1 text-xs text-red-700" data-testid="invite-delivery-failed">
            ...
            <tr>
              <td colspan="6" class="py-8 text-center text-slate-600" data-testid="invites-empty">
                {{ 'invites.empty' | t }}
              </td>
            </tr>
          }
      ...
      <p class="mt-2 text-xs text-slate-500">{{ 'invites.readonly' | t }}</p>
      @if (nextCursor()) {
        <div class="mt-3">
          <button
            pButton
      ...
      <p class="text-slate-600" data-testid="invites-failed">{{ 'card.failed' | t }}</p>
    }
  `,
})
